...
namespace LM.WebUI.Areas.Admin.Controllers
{
    public class HomeController : BaseController
    {
        ...
        public JsonResult HomeConfig(HomePageConfigModel info)
        {
            ...
            using (var homePageConfigService = ResolveService<HomePageConfigService>())
            {
                var rs = homePageConfigService.Insert(new HomePageConfig
                {
                    ...
                    AdSrc = info.AdSrc,
                    //AdLink = info.AdLink,
                    AdLink = !string.IsNullOrEmpty(info.AdLink) && !info.AdLink.Contains("http:") && !info.AdLink.Contains("https:") ? string.Concat("http://", info.AdLink) : info.AdLink,
                    SloganSrc = info.SloganSrc,
                    //SloganLink = info.SloganLink,
                    SloganLink = !string.IsNullOrEmpty(info.SloganLink) && !info.SloganLink.Contains("http:") && !info.SloganLink.Contains("https:") ? string.Concat("http://", info.SloganLink) : info.SloganLink,
                    BusinessSrc = info.BusinessSrc,
                    //BusinessLink = info.BusinessLink,
                    BusinessLink = !string.IsNullOrEmpty(info.BusinessLink) && !info.BusinessLink.Contains("http:") && !info.BusinessLink.Contains("https:") ? string.Concat("http://", info.BusinessLink) : info.BusinessLink,
                    Product1Src = info.Product1Src,
                    //Product1Link = info.Product1Link,
                    Product1Link = !string.IsNullOrEmpty(info.Product1Link) && !info.Product1Link.Contains("http:") && !info.Product1Link.Contains("https:") ? string.Concat("http://", info.Product1Link) : info.Product1Link,
                    Product2Src = info.Product2Src,
                    //Product2Link = info.Product2Link,
                    Product2Link = !string.IsNullOrEmpty(info.Product2Link) && !info.Product2Link.Contains("http:") && !info.Product2Link.Contains("https:") ? string.Concat("http://", info.Product2Link) : info.Product2Link,
                    Product3Src = info.Product3Src,
                    //Product3Link = info.Product3Link,
                    Product3Link = !string.IsNullOrEmpty(info.Product3Link) && !info.Product3Link.Contains("http:") && !info.Product3Link.Contains("https:") ? string.Concat("http://", info.Product3Link) : info.Product3Link,
                    Product4Src = info.Product4Src,
                    //Product4Link = info.Product4Link,
                    Product4Link = !string.IsNullOrEmpty(info.Product4Link) && !info.Product4Link.Contains("http:") && !info.Product4Link.Contains("https:") ? string.Concat("http://", info.Product4Link) : info.Product4Link,
                    SaveAt = DateTime.Now
                });
                if (rs.Status)
                {
                    ...
                    switch (type.ToLower())
                    {
                        case "contract":
                            ...
                            state = baseInfoService.Update(new
                            {
                                ...
                                Fax = infoModel.Fax,
                                Site = !string.IsNullOrEmpty(infoModel.Site) && !infoModel.Site.Contains("http:") && !infoModel.Site.Contains("https:") ? string.Concat("http://", infoModel.Site) : infoModel.Site,
        ...
        [HttpGet]
        [Authentication]
        public ActionResult BriefList(int pindex = 1, int psize = 2)
        {
            List<BriefModel> models;
            int itemCount;
            ...
            using (var briefService = ResolveService<BriefService>())
            {
                ...
                var rs = briefService.GetByPage("[Brief]", "SaveAt desc", pindex, psize, out itemCount);
                models = rs.Status ? rs.Data as List<BriefModel> : new List<BriefModel>();

                briefService.WriteLog(new OperationLog { User = CurrentUser.UserName, Ip = HttpContext.Request.UserHostAddress, Operation = string.Format("{1}，{0}", rs.Status ? "查询成功！" : "查询失败！", "联盟简报列表页") });
            }

            ViewBag.Briefs = models;
            ViewBag.PageInfo = new PageInfo(pindex, psize, itemCount, (itemCount % psize == 0) ? (itemCount / psize) : (itemCount / psize + 1));
            ViewBag.Nav = "BriefList";
            return View();
        }
        ...
        [HttpGet]
        [Authentication]
        public ActionResult DeleteBrief(string ids)
        {
            using (var briefService = ResolveService<BriefService>())
            {
                var rs = briefService.Delete(ids.Split(',').Select(int.Parse).ToArray());
                briefService.WriteLog(new OperationLog { User = CurrentUser.UserName, Ip = HttpContext.Request.UserHostAddress, Operation = string.Format("{1}，{0}", rs.Status ? "删除成功！" : "删除失败！", "简报删除页") });
                return RedirectToAction("BriefList");
            }
        }
        #endregion

...
namespace Dapper.Contrib.Extensions
{
    ...
    public static partial class SqlMapperExtensions
    {
        ...
        /// <summary>
        ///  Get 的 sql 语句之所以单独抽出了做成一个缓存，是因为查询的操作占据了主要的数据库操作。
        /// </summary>
        ...
        private static readonly Dictionary<string, SqlEscape> EscapeDictionary = new Dictionary<string, SqlEscape> { { "sqlconnection", new SqlEscape("[", "]") } };
        ...
        {
            var name = connection.GetType().Name.ToLower();
            return !EscapeDictionary.ContainsKey(name) ? null : EscapeDictionary[name];
        }
        ...
        /// <param name="schameName"></param>
        /// <returns></returns>
        private static string GetTableName(Type type, string schameName = "")
        {
            string name;
            if (!TypeTableName.TryGetValue(type.TypeHandle, out name))
            {
                name = string.Format("{0}{1}", schameName, type.Name);
        ...
        public static T Get<T>(this IDbConnection connection, dynamic id, IDbTransaction transaction = null, int? commandTimeout = null) where T : class
        {
            ...
            if (!GetQueries.TryGetValue(type.TypeHandle, out sql))
            {
                ...
                var tbName = escape.FormatName(type.Name);
                var keyName = escape.FormatName(onlyKey.Name);
                sql = "select * from " + tbName + " where " + keyName + " = @id";
        ...
        public static int Insert<T>(this IDbConnection connection, T entityToInsert, IDbTransaction transaction = null, int? commandTimeout = null) where T : class
        {
            ...
            for (var i = 0; i < allPropertiesExceptKeyAndComputed.Count; i++)
            {
                ...
                sbColumnList.Append(escape.FormatName(property.Name));
            ...
            var tbName = escape.FormatName(type.Name);
            return adapter.Insert(connection, transaction, commandTimeout, tbName, sbColumnList.ToString(),
        ...
        public static bool Update<T>(this IDbConnection connection, T entityToUpdate, IDbTransaction transaction = null, int? commandTimeout = null) where T : class
        {
            ...
            sb.AppendFormat("update {0} set ", escape.FormatName(type.Name));
            ...
            for (var i = 0; i < nonIdProps.Count; i++)
            {
                ...
                sb.AppendFormat("{0} = @{1}", escape.FormatName(property.Name), property.Name);
        ...
        public static bool Delete<T>(this IDbConnection connection, T entityToDelete, IDbTransaction transaction = null, int? commandTimeout = null) where T : class
        {
            ...
            var sb = new StringBuilder();
            sb.AppendFormat("delete from {0} where ", escape.FormatName(type.Name));
        ...
        public static bool DeleteAll<T>(this IDbConnection connection, IDbTransaction transaction = null, int? commandTimeout = null) where T : class
        {
            ...
            var statement = String.Format("delete from {0}", escape.FormatName(type.Name));
            var deleted = connection.Execute(statement, null, transaction, commandTimeout);
            return deleted > 0;
        }


        #region extend CRUD methods
        /// <summary>
        ///  Update entity in table "T". <br/>
        ///  Uages:
        ///     conn.Update《User》(new { Id=1, Name = "Tom" });<br/>
        ///  or 
        ///     conn.Update《User》(new User { Id=1, Name = "Tom"});<br/>
        ///  or 
        ///     conn.Update《User》(new Dictionary《string,object》{ Id = 1, Name = "Tom"});<br/>
        /// </summary>
        /// <typeparam name="T">Type to be updated</typeparam>
        /// <param name="connection">Open SqlConnection</param>
        /// <param name="entityToUpdate">
        ///     1. Entity object type of T
        ///     2. Anonumous object with has the partial properties of the entity
        ///     3. Dictionary contains partial properties of the entity
        /// </param>
        /// <param name="transaction">Transaction to be used</param>
        /// <param name="commandTimeout"></param>
        /// <returns></returns>
        public static bool Update<T>(this IDbConnection connection, object entityToUpdate, IDbTransaction transaction = null, int? commandTimeout = null) where T : class
        {
            var proxy = entityToUpdate as IProxy;
            if (proxy != null)
            {
            ...
            // 判断所传参数类是哪种：1. T ；2. anonymous object； 3. dictionary.
            if (entityToUpdate is T)
            {
                // T 类型
                var allProperties = TypePropertiesCache(type);
                var computedProperties = ComputedPropertiesCache(type);
                // check for properties have value.
                nonIdPropNames = allProperties.Except(keyProperties.Union(computedProperties)).Where(p => p.GetValue(entityToUpdate, null) != null).Select(p => p.Name);
            }
            else if (entityToUpdate is IEnumerable<KeyValuePair<string, object>>)
            {
                // 字典类型
                var obj = entityToUpdate as IEnumerable<KeyValuePair<string, object>>;
                nonIdPropNames = obj.Where(p => !keyNames.Contains(p.Key.ToLower())).Select(p => p.Key);
            }
            else
            {
                // 匿名对象
                var allProperties = entityToUpdate.GetType().GetProperties().Where(IsWriteable).ToArray();
            ...
            for (var i = 0; i < nonIdPropNames.Count(); i++)
            {
                ...
                sb.AppendFormat("{0} = @{1}", escape.FormatName(property), property);
            ...
            return connection.Execute(sb.ToString(), entityToUpdate, commandTimeout: commandTimeout, transaction: transaction) > 0;
        }

        /// <summary>
        ///  Batch delete in table "T"
        /// Usage:
        ///     conn.Delete《User》(new {1,2,3});
        /// </summary>
        ...
        /// <param name="commandTimeout"></param>
        /// <returns></returns>
        public static bool Delete<T>(this IDbConnection connection, IEnumerable<int> entityIds, IDbTransaction transaction = null, int? commandTimeout = null) where T : class
        {
            var enumerable = entityIds as int[] ?? entityIds.ToArray();
            if (entityIds == null || !enumerable.Any())
                throw new ArgumentException("Cannot Delete null Object", "entityIds");
            ...
            {
                var property = keyProperties.ElementAt(i);
                sb.AppendFormat("{0} in @{1}", escape.FormatName(property.Name), property.Name);
                if (i < keyProperties.Count - 1)
                    sb.AppendFormat(" and ");
            }

            var deleted = connection.Execute(sb.ToString(), new { Id = enumerable }, transaction, commandTimeout);
            return deleted == enumerable.Length;
        }

        /// <summary>
        ///  Batch insert insert into table "T" and returns if all the entities were inserted.
        /// </summary>
        /// <typeparam name="T"></typeparam>
        /// <param name="connection"></param>
        ...
        /// <param name="commandTimeout"></param>
        /// <returns></returns>
        public static bool Insert<T>(this IDbConnection connection, IEnumerable<T> entities, IDbTransaction transaction = null, int? commandTimeout = null) where T : class
        {
            var type = typeof(T);
            var escape = GetEscape(connection);
            ...
            for (var i = 0; i < propertiesExceptKeyAndComputed.Length; i++)
            {
                var property = propertiesExceptKeyAndComputed.ElementAt(i);
                sbColumnList.AppendFormat("{0}", escape.FormatName(property.Name));
                if (i < propertiesExceptKeyAndComputed.Length - 1)
                    sbColumnList.Append(", ");
            }
            ...
            var sql = string.Format("insert into {0} ({1}) values ({2})", escape.FormatName(type.Name), sbColumnList, sbParameterList);
            return connection.Execute(sql, entities, transaction) == entities.Count();
        }
...
/// <summary>
///  数据库格式符，区别于不同的数据库。
/// 如，sqlserver 为 "[","]"；mysql 为 "`","`"。
/// </summary>
public class SqlEscape
{
    private readonly string _leftEscape;
    private readonly string _rightEscape;

    public SqlEscape(string leftEscape, string rightEscape)
    {
        _leftEscape = leftEscape;
        _rightEscape = rightEscape;
    }

    public string FormatName(string name)
    {
        return string.Format("{0}{1}{2}", _leftEscape, name, _rightEscape);
    }
}
...
public partial class SqlServerAdapter : ISqlAdapter
{
    ...
    {
        var cmd = string.Format("insert into {0} ({1}) values ({2}); {3}", tableName, columnList, parameterList, GetLastIdentity());

        var id = connection.ExecuteScalar<int>(cmd, entityToInsert, transaction, commandTimeout);

        //NOTE: would prefer to use IDENT_CURRENT('tablename') or IDENT_SCOPE but these are not available on SQLCE
        //var r = connection.ExecuteScalar<int>("select @@IDENTITY id", transaction: transaction, commandTimeout: commandTimeout);

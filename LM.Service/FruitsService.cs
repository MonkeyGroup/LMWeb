...
{
    public class FruitsService : BaseService
    {
        private readonly FruitsRepo _fruitsRepo;

        public FruitsService(DbSession dbSession)
            : base(dbSession)
        {
            _fruitsRepo = new FruitsRepo(dbSession);
        }
        ...
        public ServiceResult GetList(string query, object param = null)
        {
            try
            {
                var models = QueryManage.GetList<FruitsModel>(query, param);
        ...
        public ServiceResult GetByPage(string targetQuery, string orderby, int pageIndex, int pageSize, out int itemCount)
        {
            try
            {
                var entities = QueryManage.GetListByPage<FruitsModel>(targetQuery, orderby, out  itemCount, pageIndex, pageSize).ToList();
        ...
        public ServiceResult Insert(Fruits entity)
        {
            try
            {
                return _fruitsRepo.Insert(entity) > 0 ?
        ...
        public ServiceResult Update(object model)
        {
            try
            {
                return _fruitsRepo.Update(model) ?
                    new ServiceResult(true, ServiceResultCode.正常, "成功") :
                    new ServiceResult(false, ServiceResultCode.服务器异常, ServiceResultCode.服务器异常.ToString());
            }
            catch (Exception e)
        ...
        public ServiceResult Delete(int id)
        {
            try
            {
                return _fruitsRepo.Delete(new Fruits { Id = id }) ?
                    new ServiceResult(true, ServiceResultCode.正常, "成功") :
                    new ServiceResult(false, ServiceResultCode.服务器异常, ServiceResultCode.服务器异常.ToString());
            }
            catch (Exception e)
        ...
        public ServiceResult Delete(int[] ids)
        {
            try
            {
                return _fruitsRepo.Delete(ids.Distinct()) ?
                    new ServiceResult(true, ServiceResultCode.正常, "成功") :
                    new ServiceResult(false, ServiceResultCode.服务器异常, ServiceResultCode.服务器异常.ToString());
            }
            catch (Exception e)
